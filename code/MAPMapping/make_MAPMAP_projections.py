...
dir_mapmap = r'/media/BigBoy/ciqle/tmp_analysis/atp1a3a/MAPMaps_atp1a3a/maps_onlybrainvoxels'
...
# file_pattern = '*_SignificantDeltaMedians.tif'
file_pattern = '*.tif'
...
os.chdir(dir_mapmap)
comp_files = glob.glob(file_pattern)
comp_files_noMixed = []
...
for file in comp_files_noMixed:
    ...
    imsave(os.path.join(dir_out, file.replace('.tif', '_projection_maxval-' + str(int(max_val)) +'_.tif')), IM_proj)
    for ch in range(3):
        sl = IM_proj[:,:,ch]
