...
"""
import cv2, warnings, time, math, sys, glob, serial, pickle
...
def plot_cum_diff_oneComp(data, data_type, treat_to_plot, cont_id, col_vec, save_name, n_norm = 5, ylim=0.8):
    ### calculate cumulative difference relative to controls, as in Randlett et al., Current Biology, 2019
    ### this is done for one component (eg Probability of Response), plotted together across conditions
    ...
    if data_type == 'OBendLatencies': # invert so that habituation changes match direction
        ...
        data_to_plot = abs(data[data_type][stim_given==1, :])
    for treat_id in treat_to_plot:
        if not treat_id == cont_id:
            treat_ids = rois[treat_id]
            treat_data = data_to_plot[:,treat_ids]
            n_treat = len(treat_ids)
            cont_ids = rois[cont_id]
            ...
            CI[0][np.isnan(CI[0])] = mu[np.isnan(CI[0])]
            CI[1][np.isnan(CI[1])] = mu[np.isnan(CI[1])]
            plt.plot(np.arange(240), mu, color=col_vec[treat_id], label=names[treat_id])
            plt.fill_between(np.arange(240), CI[0], CI[1], alpha=0.3, color=col_vec[treat_id], label='_nolegend_', interpolate=True)
    plt.ylabel('Cum. Mean Diff\nvs. Control')
    plt.xlabel('Stimuli')
    plt.legend(bbox_to_anchor=(1.05, 1.0, 0.3, 0.2), loc='upper left')
    # plt.legend(['probability', 
